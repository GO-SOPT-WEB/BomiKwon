...
const CardList = () => {
  ...
  const [flippedCardIDList, setFlippedCardIDList] = useState<string[]>([]); //뒤집어진 카드의 id를 저장
  const [correctCardList, setCorrectCardList] = useState<string[]>([]); //짝이 맞는 쌍을 담은 배열
   ...
   */
  const handleClickedCards = (card: string, cardID: string) => {
